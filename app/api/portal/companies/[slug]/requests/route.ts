...
export async function POST(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (!engagementName) return NextResponse.json({ error: "Workspace or property name is required" }, { status: 400 });

    const ignoredKeys = new Set([
      "projectTitle", "title", "workOrderTitle", "projectName", "engagementName", "engagementId", "propertyName",
      "priority", "service", "type", "packageId", "packageTitle",
    ]);
    ...
    const result = await prisma.$transaction(async (tx) => {
      let engagement = body?.engagementId
        ? await tx.engagement.findFirst({ where: { id: Number(body.engagementId), companyId: company.id } })
        : null;
      ...
      if (!engagement) {
        ...
        engagement = await tx.engagement.findFirst({
          where: {
            ...
            OR: [
              ...
              { name: { equals: engagementName, mode: "insensitive" as const } },
            ],
          },
        });
        ...
        if (!engagement) {
          ...
          let suffix = 2;
          while (await tx.engagement.findUnique({ where: { companyId_slug: { companyId: company.id, slug: uniqueSlug } } })) {
          ...
          engagement = await tx.engagement.create({
            data: {
              ...
              name: engagementName,
              slug: uniqueSlug,
              type: address ? "property" : String(body?.engagementType || "campaign"),
              address,
              city: body?.city ? String(body.city).trim() : null,
              state: body?.state ? String(body.state).trim() : null,
              postalCode: body?.postalCode ? String(body.postalCode).trim() : null,
              description: body?.engagementDescription ? String(body.engagementDescription).trim() : null,
            },
          });
        }
      }

      const marketingRequest = await tx.marketingRequest.create({
        data: { companyId: company.id, type: service, title: workOrderTitle, description: description || null, priority, status: "converted" },
      });
      const project = await tx.project.create({
        data: {
          ...
          description: description || `${service} work order submitted through the client workspace.`,
          status: "active",
          priority,
          startDate: createdAt,
          dueDate,
          ...
          tasks: {
            create: workflow.map((step, index) => {
              ...
              return {
                ...
                status: index === 0 ? "complete" : index === 1 ? "in_progress" : "todo",
                priority,
                dueDate: taskDueDate,
                sortOrder: index,
              };
          ...
          activities: {
            create: [
              { type: "project_created", message: `${service} work order added to ${engagement.name}.`, actor: "UPZ Workflow Engine" },
              { type: "status_changed", message: `Workflow started with ${workflow.length} production stages.`, actor: "UPZ Workflow Engine" },
            ],
          },
        },
