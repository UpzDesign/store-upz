...
export async function POST(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const ignoredKeys = new Set(["projectTitle", "priority", "service", "type", "packageId", "packageTitle"]);
    ...
    const result = await prisma.$transaction(async (tx) => {
      const marketingRequest = await tx.marketingRequest.create({
        data: { companyId: company.id, type: service, title: projectTitle, description: description || null, priority, status: "converted" },
      });
      const project = await tx.project.create({
        data: {
          ...
          title: projectTitle,
          description: description || `${service} project submitted through the client portal.`,
          status: "active",
          priority,
          startDate: createdAt,
          dueDate,
          ...
          activities: {
            create: [
              { type: "project_created", message: `Project created automatically from the ${service} request.`, actor: "UPZ Workflow Engine" },
      ...
      return { request: marketingRequest, project };
    });
