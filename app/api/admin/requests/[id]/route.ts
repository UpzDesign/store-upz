...
export async function GET(_request:NextRequest,context:{params:Promise<{id:string}>}){
  try{
    ...
    if(groupId){
      ...
      requestGroup={
        id:groupId,
        projectName:String(itemContext?.requestGroup?.projectName||itemContext?.engagementName||itemContext?.portfolioName||""),
        propertyType:String(itemContext?.requestGroup?.propertyType||itemContext?.answers?.propertyType||""),
        address:String(itemContext?.requestGroup?.address||itemContext?.propertyAddress||""),
        space:String(itemContext?.requestGroup?.space||itemContext?.locationName||""),
        services:related.map(({entry})=>({id:entry.id,name:entry.type,title:entry.title,status:entry.status,current:entry.id===item.id})),
      };
    }

    return NextResponse.json({...item,company,project,requestGroup},{headers:{"Cache-Control":"no-store, max-age=0"}});
...
export async function DELETE(_request:NextRequest,context:{params:Promise<{id:string}>}){
  try{
    ...
    const existing=await prisma.marketingRequest.findUnique({where:{id:requestId},select:{id:true,project:{select:{id:true,engagementId:true}}}});
    if(!existing)return NextResponse.json({error:"Request not found"},{status:404});

    const projectId=existing.project?.id||null,engagementId=existing.project?.engagementId||null;
    await prisma.$transaction(async(tx)=>{
      if(projectId)await tx.project.delete({where:{id:projectId}});
      await tx.marketingRequest.delete({where:{id:requestId}});
      if(engagementId){const[remainingOrders,remainingAssets]=await Promise.all([tx.project.count({where:{engagementId}}),tx.engagementAsset.count({where:{engagementId}})]);if(remainingOrders===0&&remainingAssets===0)await tx.engagement.delete({where:{id:engagementId}})}
    });
