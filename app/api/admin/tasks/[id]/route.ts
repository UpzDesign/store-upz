...
export async function PATCH(request:NextRequest,context:{params:Promise<{id:string}>}){
 ...
 if(session.role==="admin"&&body.sortOrder!==undefined){
  const siblings=await prisma.projectTask.findMany({where:{projectId:existing.projectId},orderBy:{sortOrder:"asc"}});
  const withoutCurrent=siblings.filter(item=>item.id!==taskId);
  const target=Math.max(0,Math.min(withoutCurrent.length,Number(body.sortOrder)||0));
  withoutCurrent.splice(target,0,existing);
  await prisma.$transaction(withoutCurrent.map((item,index)=>prisma.projectTask.update({where:{id:item.id},data:{sortOrder:index}})));
 }

 const contributorData=session.role==="contributor"
 ...
 let message=`Work stage updated: ${task.title}.`;
