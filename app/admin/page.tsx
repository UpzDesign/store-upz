...
export default function AdminPage() {
  ...
  useEffect(()=>{if(!authenticated)return;setLoadingCompanies(true);setCompanyError("");Promise.all([
    fetch("/api/admin/companies").then((response)=>{if(!response.ok)throw new Error("Unable to load companies");return response.json();}),
    fetch("/api/admin/inbox").then((response)=>response.ok?response.json():[]),
  ...
  return <main className="admin-page"><section className="admin-shell">
    <aside className="admin-sidebar"><img src="/upz-logo.svg" alt="UPZ Design"/><nav><a href="#dashboard">Dashboard</a><Link href="/admin/inbox">Inbox {totalNew>0?`(${totalNew})`:""}</Link><a href="#companies">Companies</a><a href="#integrations">Integrations</a></nav></aside>
    <div className="admin-main">
      ...
      <section className="admin-action-required"><div><span>{totalNew>0?"Action Required":"Inbox Clear"}</span><h2>{totalNew>0?`${totalNew} new request${totalNew===1?"":"s"} awaiting review`:"No new requests"}</h2><p>{totalNew>0?"Requests remain highlighted until reviewed, advanced, or converted into a project.":"New client activity will appear here automatically."}</p></div><Link className="admin-primary-button" href="/admin/inbox">Open Admin Inbox</Link></section>

      {inbox.length>0&&<section className="admin-section admin-dashboard-inbox"><div className="admin-section-heading"><div><span>Priority Inbox</span><h2>Latest requests</h2></div><Link className="admin-secondary-button" href="/admin/inbox">View All</Link></div><div className="admin-inbox-list">{inbox.slice(0,4).map((item)=>{const isNew=!item.project||item.project.status.toLowerCase()==="new";return <article key={item.id} className={`admin-inbox-row ${isNew?"is-new":""} priority-${item.priority}`}><div className="admin-inbox-company"><div><strong>{item.company.shortName}</strong><span>{item.company.name}</span></div></div><div className="admin-inbox-copy"><div className="admin-inbox-tags"><b>{isNew?"New Request":item.project?.status}</b><em>{item.priority} priority</em><span>{item.type}</span></div><h3>{item.title}</h3><small>{new Date(item.createdAt).toLocaleString()}</small></div><div className="admin-inbox-actions"><Link className="admin-primary-button" href={`/admin/request/${item.id}`}>Review</Link></div></article>;})}</div></section>}

      <section id="companies" className="admin-section"><div className="admin-section-heading"><div><span>Companies</span><h2>Active client portals</h2></div></div>{loadingCompanies&&<p>Loading companies...</p>}{companyError&&<p className="admin-error">{companyError}</p>}<div className="admin-company-grid">{companies.map((company)=>{const count=Number(company.newRequestCount||0);return <article key={company.id} className={`admin-company-card ${count>0?"has-new-requests":""}`}>{count>0&&<Link className="admin-request-badge" href={`/admin/company/${company.slug}/requests`}>{count} new request{count===1?"":"s"}</Link>}<div className="admin-company-logo" style={{borderColor:company.primaryColor}}><img src={company.logo||"/upz-logo.svg"} alt={`${company.name} logo`}/></div><div><span style={{color:company.primaryColor}}>{company.shortName}</span><h3>{company.name}</h3><p>{company.heroText}</p></div><div className="admin-company-meta"><div><strong>Status</strong><span>{company.portalEnabled?"Active":"Disabled"}</span></div><div><strong>Requests</strong><Link href={`/admin/company/${company.slug}/requests`}>{count?`${count} need attention`:"View Requests"}</Link></div><div><strong>Manage</strong><Link href={`/admin/company/${company.slug}`}>Company Settings</Link></div><div><strong>Portal</strong><Link href={`/portal/${company.slug}`}>Open Portal</Link></div></div></article>;})}</div></section>
