...
export default function OperationsPage() {
  ...
  const load = () => fetch("/api/admin/operations", { cache:"no-store" }).then(r=>r.json()).then((value:Data) => {
    ...
    setData(normalized);
    if (selected) {
      const updated = normalized.projects.find(project => project.id === selected.id);
      if (updated) { setSelected(updated); setDraft({ ...updated, startDate:dateInput(updated.startDate), dueDate:dateInput(updated.dueDate) }); }
    }
  });
  ...
  function open(project:Project) { setSelected(project); setDraft({ ...project, status:normalizedStatus(project.status), startDate:dateInput(project.startDate), dueDate:dateInput(project.dueDate) }); }
  ...
  async function removeWorkOrder() {
    ...
    if(!window.confirm(`Permanently remove “${selected.title}”? It will disappear from Operations, Timeline, and the client Projects & Campaigns page.`)) return;
    setDeleting(true);
    const response = await fetch(`/api/admin/work-orders/${selected.id}`, { method:"DELETE" });
    setDeleting(false);
  ...
  return <main className="ops-page">
    <header className="ops-head"><div><span>UPZ WORKSPACE</span><h1>Operations</h1><p>Manage work orders, assignments, tasks, dates, progress, approvals, and production status.</p></div><Link href="/admin/team">Manage team →</Link></header>
    ...
    <div className="ops-toolbar"><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search work orders, companies, engagements..."/><select value={assignee} onChange={event=>setAssignee(event.target.value)}><option value="all">All team members</option>{data.team.map(member=><option value={member.name} key={member.id}>{member.name}</option>)}</select><select value={priority} onChange={event=>setPriority(event.target.value)}><option value="all">All priorities</option>{PRIORITIES.map(value=><option value={value} key={value}>{value}</option>)}</select><button className={attentionOnly?"active":""} onClick={()=>setAttentionOnly(!attentionOnly)}>Needs attention</button></div>
