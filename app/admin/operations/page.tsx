...
export default function OperationsPage() {
  ...
  const open=(project:Project)=>{setSelected(project);setDraft({...project,status:normalizeProjectStage(project.status),startDate:dateInput(project.startDate),dueDate:dateInput(project.dueDate)})};
  const load = (projectId=requestedProject) => fetch("/api/admin/operations", { cache:"no-store" }).then(r=>r.json()).then((value:Data) => {
    const normalized = { ...value, projects:value.projects.map(project => ({ ...project, status:normalizeProjectStage(project.status) })) };
    setData(normalized);
  ...
  return <main className="ops-page">
    ...
    {tab==="approvals"&&<section className="ops-approval-list">{projects.filter(project=>["waiting_client","review"].includes(normalizeProjectStage(project.status))).map(project=><article key={project.id}><div><small>{project.company.shortName}</small><h3>{project.title}</h3></div><button onClick={()=>open(project)}>Manage</button></article>)}</section>}
