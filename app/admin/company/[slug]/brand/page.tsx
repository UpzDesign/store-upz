...
export default function CompanyBrandPage(){
 ...
 const[company,setCompany]=useState<Company|null>(null),[saving,setSaving]=useState(false),[message,setMessage]=useState(""),[uploadError,setUploadError]=useState("");
 useEffect(()=>{fetch(`/api/admin/companies/${slug}`,{cache:"no-store"}).then(r=>r.json()).then(setCompany)},[slug]);
 ...
 function applyContrast(mode:string){if(mode==="auto")set("logoTextColor",autoContrast(company.primaryColor));if(mode==="light")set("logoTextColor","#ffffff");if(mode==="dark")set("logoTextColor","#111111");}
 function uploadLogo(file?:File){if(!file)return;setUploadError("");if(!FILE_TYPES.includes(file.type)){setUploadError("Use SVG, PNG, or WebP.");return}if(file.size>1.5*1024*1024){setUploadError("Logo must be 1.5 MB or smaller.");return}const reader=new FileReader();reader.onload=()=>{set("logo",String(reader.result||""));set("logoType","image")};reader.onerror=()=>setUploadError("Unable to read logo file.");reader.readAsDataURL(file)}
 async function save(e:React.FormEvent){e.preventDefault();setSaving(true);setMessage("");const r=await fetch(`/api/admin/companies/${slug}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(company)}),d=await r.json();setSaving(false);setMessage(r.ok?"Brand and contrast settings saved.":d?.error||"Unable to save");if(r.ok)setCompany(d)}
 const preview={...company,logoText:company.logoText||company.shortName};
 return <AdminPage className="company-brand-page"><AdminHeader eyebrow="Company Settings" title={`${company.name} Brand`} description="Use the same logo, colors, and contrast rules that power the client portal and new-client setup." actions={<AdminButton href={`/admin/company/${slug}`}>Back to Company</AdminButton>}/><form onSubmit={save}><AdminSection><AdminSectionHeader eyebrow="Identity" title="Logo, color, and contrast"/><AdminFormGrid>
  <AdminField label="Logo Type"><select value={company.logoType} onChange={e=>set("logoType",e.target.value)}><option value="image">Image logo</option><option value="text">Text wordmark</option><option value="none">No logo</option></select></AdminField>
  {company.logoType==="image"&&<><AdminField label="Upload Logo" className="wide"><input type="file" accept=".svg,.png,.webp,image/svg+xml,image/png,image/webp" onChange={e=>uploadLogo(e.target.files?.[0])}/><small>SVG, PNG, or WebP · max 1.5 MB. Transparent files work best.</small>{uploadError&&<small className="admin-error">{uploadError}</small>}</AdminField><AdminField label="Logo URL / Path" className="wide"><input value={company.logo||""} onChange={e=>set("logo",e.target.value)} placeholder="Optional fallback URL or path"/></AdminField></>}
  {company.logoType==="text"&&<><AdminField label="Wordmark Text"><input value={company.logoText||company.shortName} onChange={e=>set("logoText",e.target.value)}/></AdminField><AdminField label="Wordmark Style"><select value={company.logoFontStyle||"sans"} onChange={e=>set("logoFontStyle",e.target.value)}><option value="sans">Sans Bold</option><option value="serif">Serif</option><option value="light">Sans Light</option><option value="condensed">Condensed Bold</option></select></AdminField></>}
  <AdminField label="Primary Brand Color"><input type="color" value={company.primaryColor} onChange={e=>{set("primaryColor",e.target.value);if(contrastMode==="auto")set("logoTextColor",autoContrast(e.target.value))}}/></AdminField><AdminField label="Secondary Color"><input type="color" value={company.secondaryColor} onChange={e=>set("secondaryColor",e.target.value)}/></AdminField>
  <AdminField label="On-Brand / Wordmark Color" className="wide"><div className="admin-brand-contrast-actions"><button type="button" className={contrastMode==="auto"?"active":""} onClick={()=>applyContrast("auto")}>Auto Contrast</button><button type="button" className={contrastMode==="light"?"active":""} onClick={()=>applyContrast("light")}>Light</button><button type="button" className={contrastMode==="dark"?"active":""} onClick={()=>applyContrast("dark")}>Dark</button><label>Custom<input type="color" value={company.logoTextColor||autoContrast(company.primaryColor)} onChange={e=>set("logoTextColor",e.target.value)}/></label></div><small>Used for buttons, wordmarks, and text placed on the primary brand color.</small></AdminField>
 </AdminFormGrid>
 <div className="admin-brand-preview-shell"><div className="admin-brand-logo-preview"><span>Logo preview</span><div className="admin-brand-logo-stage"><CompanyLogo company={preview}/></div></div><div className="admin-brand-color-preview" style={{background:company.primaryColor,color:company.logoTextColor||autoContrast(company.primaryColor)}}><span>Brand contrast</span><strong>{company.logoText||company.shortName}</strong><p>Buttons and text on your primary brand color will use this contrast treatment.</p></div></div>
