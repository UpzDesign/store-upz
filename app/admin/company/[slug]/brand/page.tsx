...
export default function CompanyBrandPage(){
 ...
 return <AdminPage className="company-brand-page"><AdminHeader eyebrow="Company Settings" title={`${currentCompany.name} Brand`} description="Use the same logo, colors, and contrast rules that power the client portal and new-client setup." actions={<AdminButton href={`/admin/company/${slug}`}>Back to Company</AdminButton>}/><form onSubmit={save}><AdminSection><AdminSectionHeader eyebrow="Identity" title="Logo, color, and contrast"/><AdminFormGrid>
  ...
  {currentCompany.logoType==="image"&&<><AdminField label="Upload Logo" className="wide"><input type="file" accept=".svg,.png,.webp,image/svg+xml,image/png,image/webp" onChange={e=>uploadLogo(e.target.files?.[0])}/><small>SVG, PNG, or WebP · max 1.5 MB. Transparent files work best.</small>{uploadError&&<small className="admin-error">{uploadError}</small>}</AdminField><AdminField label="Logo URL / Path" className="wide"><input value={currentCompany.logo||""} onChange={e=>set("logo",e.target.value)} placeholder="Optional fallback URL or path"/></AdminField></>}
