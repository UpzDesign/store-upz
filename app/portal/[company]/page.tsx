...
function itemMatches(item:CatalogItem,keywords:string[]){ const haystack=`${item.title} ${item.description||""} ${item.collection?.name||""}`.toLowerCase(); return keywords.some((keyword)=>haystack.includes(keyword)); }
function serviceRoute(item:CatalogItem){ return SERVICE_FOCUS.find((bucket)=>itemMatches(item,bucket.keywords))?.key || "general"; }
...
function CatalogCard({item,company,addItem}:{item:CatalogItem;company:PortalCompany;addItem:(item:CartItem)=>void}){
  ...
  const serviceHref=item.itemType==="service"?`/portal/${company.slug}/request/${serviceRoute(item)}`:null;
  const href=productHref || serviceHref;
  const content=<><div className="portal-product-card-image"><img src={getItemImage(item)} alt={item.title}/></div><div className="portal-product-card-body"><span>{getItemLabel(item)} · {item.collection?.name||"Catalog"}</span><h3>{company.shortName} {item.title}</h3><strong>{formatPrice(item.price)}</strong>{item.description&&<p>{item.description}</p>}<div className="portal-product-card-action">{serviceHref?"Start Project":productHref?"View Product":"Add to Cart"}<b>→</b></div></div></>;
