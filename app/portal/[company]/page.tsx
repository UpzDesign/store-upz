...
function ServiceCard({service,company}:{service:IntakeDefinition;company:PortalCompany}){
  return <Link href={`/portal/${company.slug}/request/${service.slug}`} className="portal-assigned-service-card"><span>Available service</span><h3>{service.name}</h3><p>{service.description}</p><strong>Start project →</strong></Link>;
}
...
export default function CompanyPortalPage(){
  ...
  return <main className="portal-page" style={{"--company-primary":company.primaryColor,"--company-secondary":company.secondaryColor} as React.CSSProperties}>
    ...
    <section id="services" className="portal-section portal-service-lead"><div className="upz-wrap"><div className="portal-section-heading"><div><span>Company Catalog</span><h2>My services</h2></div></div>{services.length?<div className="portal-assigned-service-grid">{services.map((service)=><ServiceCard key={service.slug} service={service} company={company}/>)}</div>:<div className="portal-empty-service"><h3>{loading?"Loading services...":"No services enabled"}</h3><p>Contact UPZ to add services to this workspace.</p></div>}</div></section>

    <section className="portal-section portal-service-lead"><div className="upz-wrap"><div className="portal-section-heading"><div><span>Curated Solutions</span><h2>Packages</h2></div><Link href={`/portal/${company.slug}/packages`}>View All Packages</Link></div><div className="portal-service-priority-grid">{featuredPackages.length?featuredPackages.map((pkg)=><Link key={pkg.id} href={`/portal/${company.slug}/packages`}><span>Featured package</span><h3>{pkg.title}</h3><p>{pkg.description||"A curated combination of services, products, and deliverables."}</p></Link>):<Link href={`/portal/${company.slug}/packages`}><span>Curated solutions</span><h3>Browse Packages</h3><p>Start a complete project with a pre-built combination of services and products.</p></Link>}</div></div></section>
