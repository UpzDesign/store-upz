...
function CatalogCard({item,company,addItem}:{item:CatalogItem;company:PortalCompany;addItem:(item:CartItem)=>void}){
  ...
  if(productHref) return <Link href={productHref} className="portal-product-card portal-product-card-link">{content}</Link>;
...
function ServiceCard({service,company}:{service:IntakeDefinition;company:PortalCompany}){
  return <Link href={`/portal/${company.slug}/request/${service.slug}`} className="portal-premium-service-card"><span>Creative Service</span><h3>{service.name}</h3><p>{service.description}</p><strong>Start project <b>→</b></strong></Link>;
}
...
export default function CompanyPortalPage(){
  ...
  useEffect(()=>{ if(!company)return; setLoading(true); Promise.all([
    ...
    fetch(`/api/portal/companies/${company.slug}/packages`).then((res)=>res.ok?res.json():[]),
    fetch(`/api/portal/companies/${company.slug}/services`,{cache:"no-store"}).then((res)=>res.ok?res.json():[]),
  ]).then(([catalog,requestData,packageData,serviceData])=>{setCatalogItems(Array.isArray(catalog)?catalog:[]);setRequests(Array.isArray(requestData)?requestData:[]);setPackages(Array.isArray(packageData)?packageData:[]);setServices(Array.isArray(serviceData)?serviceData:[]);}).catch(()=>{setCatalogItems([]);setPackages([]);setServices([]);}).finally(()=>setLoading(false)); },[company]);
  ...
  return <main className="portal-page" style={{"--company-primary":company.primaryColor,"--company-secondary":company.secondaryColor} as React.CSSProperties}>
    <section className="portal-hero"><div className="upz-wrap portal-hero-inner"><div><div className="portal-eyebrow">Private Company Portal</div><h1>{company.heroTitle}</h1><p>{company.heroText}</p><div className="portal-actions"><a href="#requests">Active Projects</a><a href="#services">Services</a><Link href={`/portal/${company.slug}/packages`}>Packages</Link><a href="#brand-assets">Brand Assets</a></div></div><div className="portal-brand-card"><img src={company.logo||"/upz-logo.svg"} alt={`${company.name} logo`}/><h2>{company.name}</h2><p>Approved creative services, projects, packages, merchandise, and brand resources.</p></div></div></section>
    ...
    <section id="services" className="portal-section portal-premium-services"><div className="upz-wrap"><div className="portal-section-heading portal-dark-heading"><div><span>Creative Capabilities</span><h2>Available services</h2><p>Select a service to start a new project with the correct intake and workflow.</p></div></div>{services.length?<div className="portal-premium-service-grid">{services.map((service)=><ServiceCard key={service.slug} service={service} company={company}/>)}</div>:<div className="portal-empty-service portal-empty-dark"><h3>{loading?"Loading services...":"No services enabled"}</h3><p>Contact UPZ to add services to this workspace.</p></div>}</div></section>

    <section className="portal-section portal-featured-packages"><div className="upz-wrap"><div className="portal-section-heading"><div><span>Curated Solutions</span><h2>Project packages</h2><p>Pre-built combinations of services and deliverables for common project needs.</p></div><Link href={`/portal/${company.slug}/packages`}>View All Packages</Link></div><div className="portal-featured-package-grid">{featuredPackages.length?featuredPackages.map((pkg)=><Link key={pkg.id} href={`/portal/${company.slug}/packages`}><span>Featured package</span><h3>{pkg.title}</h3><p>{pkg.description||"A curated combination of services, products, and deliverables."}</p><strong>Explore package →</strong></Link>):<Link href={`/portal/${company.slug}/packages`}><span>Curated solutions</span><h3>Browse Packages</h3><p>Start a complete project with a pre-built combination of services and products.</p><strong>View packages →</strong></Link>}</div></div></section>
