...
export default function CompanyPortalPage() {
  ...
  const collections = useMemo(() => {
    ...
    catalogItems.forEach((item) => unique.set(item.collection?.name || "Unassigned", item.collection?.name || "Unassigned"));
  ...
  const productItems = useMemo(() => filteredItems.filter((item) => item.itemType === "product" || item.sourceVendor === "printful"), [filteredItems]);
  const serviceItems = useMemo(() => filteredItems.filter((item) => item.itemType === "service"), [filteredItems]);
  const digitalItems = useMemo(() => filteredItems.filter((item) => ["digital", "asset", "custom"].includes(item.itemType)), [filteredItems]);
    ...
    <main className="portal-page" style={{ "--company-primary": company.primaryColor, "--company-secondary": company.secondaryColor } as React.CSSProperties}>
      <section className="portal-hero">
        <div className="upz-wrap portal-hero-inner">
          <div>
            ...
            <div className="portal-actions"><a href="#products">Products</a><a href="#services">Services</a><a href="#assets">Brand Assets</a></div>
          </div>
          <div className="portal-brand-card"><img src={company.logo || "/upz-logo.svg"} alt={`${company.name} logo`} /><h2>{company.name}</h2><p>Approved materials, company merchandise, and marketing support powered by UPZ Design.</p></div>
        </div>
      </section>
      ...
      <section className="portal-section">
        <div className="upz-wrap">
          <div className="portal-section-heading"><div><span>Browse</span><h2>Portal Catalog</h2></div><strong>{loading ? "Loading..." : `${filteredItems.length} items`}</strong></div>
          <div className="portal-filter-row">{collections.map((collection) => <button key={collection} className={activeCollection === collection ? "is-active" : ""} onClick={() => setActiveCollection(collection)}>{collection}</button>)}</div>
        </div>
      </section>
      ...
      <section id="products" className="portal-section">
        <div className="upz-wrap">
          <div className="portal-section-heading"><div><span>Products</span><h2>Merchandise & Print Items</h2></div><strong>{productItems.length} items</strong></div>
          {productItems.length ? <div className="portal-product-grid">{productItems.map((item) => <CatalogCard key={item.id} item={item} company={company} addItem={addItem} />)}</div> : <p>No products are currently available in this section.</p>}
        </div>
      </section>

      <section id="services" className="portal-section portal-services-section">
        <div className="upz-wrap">
          <div className="portal-section-heading"><div><span>Services</span><h2>Marketing & Creative Services</h2></div><strong>{serviceItems.length} services</strong></div>
          {serviceItems.length ? <div className="portal-product-grid">{serviceItems.map((item) => <CatalogCard key={item.id} item={item} company={company} addItem={addItem} />)}</div> : <p>No services are currently available in this section.</p>}
        </div>
      </section>

      {digitalItems.length > 0 && <section id="digital" className="portal-section"><div className="upz-wrap"><div className="portal-section-heading"><div><span>Digital</span><h2>Assets & Downloads</h2></div><strong>{digitalItems.length} items</strong></div><div className="portal-product-grid">{digitalItems.map((item) => <CatalogCard key={item.id} item={item} company={company} addItem={addItem} />)}</div></div></section>}
