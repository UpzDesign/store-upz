...
export default function CompanyPortalPage() {
  ...
  return (
    <main className="portal-page" style={{ "--company-primary": company.primaryColor, "--company-secondary": company.secondaryColor } as React.CSSProperties}>
      <section className="portal-hero"><div className="upz-wrap portal-hero-inner"><div><div className="portal-eyebrow">Private Company Portal</div><h1>{company.heroTitle}</h1><p>{company.heroText}</p><div className="portal-actions"><a href="#services">Request Services</a><a href="#signage-print-installation">Signage & Print</a><a href="#merchandise">Merchandise</a></div></div><div className="portal-brand-card"><img src={company.logo || "/upz-logo.svg"} alt={`${company.name} logo`} /><h2>{company.name}</h2><p>Approved marketing services, creative requests, print production, and company merchandise powered by UPZ Design.</p></div></div></section>
      <section className="portal-modules"><div className="upz-wrap portal-module-grid">{["Signage & Print", "Photography", "Brochure Design", "Web Development", "Merchandise", "Brand Assets"].map((module) => <a key={module} href={`#${slugLabel(module)}`} className="portal-module-card"><span>{module}</span></a>)}</div></section>

      <section id="services" className="portal-section portal-service-lead"><div className="upz-wrap"><div className="portal-section-heading"><div><span>Start a project</span><h2>Marketing services first</h2></div><strong>{loading ? "Loading..." : `${serviceItems.length} services`}</strong></div><div className="portal-service-priority-grid">{SERVICE_FOCUS.map((bucket) => <a key={bucket.key} href={`#${bucket.key}`}><span>{bucket.items.length} available</span><h3>{bucket.title}</h3><p>{bucket.text}</p></a>)}</div></div></section>
